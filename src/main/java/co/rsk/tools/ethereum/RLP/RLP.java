 ...
 */
package co.rsk.tools.ethereum.RLP;
...
public class RLP {
    ...
    static void fullTraverse(byte[] msgData, int level, int startPos,
        ...
        try {
            ...
            throw new RuntimeException("RLP wrong encoding (" + ByteUtil.toHexString(msgData, startPos, endPos - startPos) + ")", e);
        } catch (OutOfMemoryError e) {
            throw new RuntimeException("Invalid RLP (excessive mem allocation while parsing) (" + ByteUtil.toHexString(msgData, startPos, endPos - startPos) + ")", e);
        }
    }
    ...
    public static byte[] encode(Object input) {
        ...
        if (val.isList()) {
            ...
            for (Object object : inputArray) {
                output = Arrays.concatenate(output, encode(object));
            }
            byte[] prefix = encodeLength(output.length, OFFSET_SHORT_LIST);
            return Arrays.concatenate(prefix, output);
        } else {
            ...
            if (inputAsBytes.length == 1 && (inputAsBytes[0] & 0xff) <= 0x80) {
                ...
                return Arrays.concatenate(firstByte, inputAsBytes);
            }
        }
    }
    ...
    public static byte[] encodeBigInteger(BigInteger srcBigInteger) {
        ...
        if (srcBigInteger.equals(BigInteger.ZERO))
            ...
            return encodeElement(Arrays.asUnsignedByteArray(srcBigInteger));
    }

    public static byte[] encodeElement(byte[] srcData) {

        // [0x80]
        if (ByteUtil2.isNullOrZeroArray(srcData)) {
            return new byte[]{(byte) OFFSET_SHORT_ITEM};
    ...
    private static byte[] toBytes(Object input) {
        if (input instanceof byte[]) {
            ...
            return (inputLong == 0) ? ByteUtil.EMPTY_BYTE_ARRAY : Arrays.asUnsignedByteArray(BigInteger.valueOf(inputLong));
        } else if (input instanceof Integer) {
            Integer inputInt = (Integer) input;
            return (inputInt == 0) ? ByteUtil.EMPTY_BYTE_ARRAY : Arrays.asUnsignedByteArray(BigInteger.valueOf(inputInt));
        } else if (input instanceof BigInteger) {
            BigInteger inputBigInt = (BigInteger) input;
            return (inputBigInt.equals(BigInteger.ZERO)) ? ByteUtil.EMPTY_BYTE_ARRAY : Arrays.asUnsignedByteArray(inputBigInt);
