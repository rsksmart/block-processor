 ...
 */
package co.rsk.tools.ethereum.trie;

import co.rsk.tools.ethereum.RLP.RLP;
...
import static co.rsk.tools.ethereum.RLP.RLP.*;
...
public class TrieImpl implements Trie<byte[]> {
    ...
    public final class Node {
        ...
        public String dumpStruct(String indent, String prefix) {
            String ret = indent + prefix + getType() + (dirty ? " *" : "") +
                    (hash == null ? "" : "(hash: " + ByteUtil.toHexString(hash).substring(0, 6) + ")");
            if (getType() == NodeType.BranchNode) {
                byte[] value = branchNodeGetValue();
                ret += (value == null ? "" : " [T] = " + ByteUtil.toHexString(value)) + "\n";
                for (int i = 0; i < 16; i++) {
                    Node child = branchNodeGetChild(i);
                    if (child != null) {
                ...
                ret += " [" + kvNodeGetKey() + "] = " + ByteUtil.toHexString(kvNodeGetValue()) + "\n";
            }
            return ret;
        }
    ...
    public void setRoot(byte[] root) {
        if (root != null && !FastByteComparisons.equalBytes(root, EMPTY_TRIE_HASH)) {
    ...
    public boolean equals(Object o) {
        ...
        return FastByteComparisons.equalBytes(getRootHash(), trieImpl1.getRootHash());
