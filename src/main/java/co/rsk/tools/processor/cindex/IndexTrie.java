...
public class IndexTrie {
    ...
    public IndexTrie put(byte[] key, int value) {
        byte[] keySlice = PackedTrieKeySlice.fromKey(key);
    ...
    private void collectKeys(Set<ByteArrayWrapper> set, byte[] key, int collectKeyLen) {
        if (collectKeyLen != Integer.MAX_VALUE && PackedTrieKeySlice.length(key) > collectKeyLen) {
            return;
        }

        boolean shouldCollect = collectKeyLen == Integer.MAX_VALUE ||
                PackedTrieKeySlice.length(key) == collectKeyLen;
        if (getValue() >= 0 && shouldCollect) {
            // convert bit string into byte[]
            set.add(new ByteArrayWrapper(PackedTrieKeySlice.encode(key)));
        }
        ...
        for (byte k = 0; k < 2; k++) {
            ...
            byte[] nodeKey = PackedTrieKeySlice.rebuildSharedPath(key,k, node.getSharedPath());
    ...
    private IndexTrie internalFind(byte[] packedKey) {
        ...
        int commonPathLength = PackedTrieKeySlice.length(commonPath);
        if (commonPathLength < PackedTrieKeySlice.length(sharedPath)) {
            return null;
        }

        if (commonPathLength == PackedTrieKeySlice.length(packedKey)) {
            return this;
        }

        IndexTrie node = this.retrieveNode(PackedTrieKeySlice.get(packedKey,commonPathLength));
        if (node == null) {
            return null;
        }

        return node.internalFind(PackedTrieKeySlice.slice(packedKey,commonPathLength + 1,
                PackedTrieKeySlice.length(packedKey)));
    }
    ...
    public IndexTrie getRight() {
     ...
     static IndexTrie coalesce(IndexTrie trie) {
        ...
        byte[] newSharedPath = PackedTrieKeySlice.rebuildSharedPath(
                trie.getSharedPath(),childImplicitByte, child.getSharedPath());
        return newIndexTrie(newSharedPath, child.getValue(), child.getLeft(), child.getRight());
    }

    public static IndexTrie newIndexTrie( byte[] newSharedPath,
                                       int value,IndexTrie left,IndexTrie right) {
        int pathLength = PackedTrieKeySlice.length(newSharedPath);
     ...
     *
     */
    private IndexTrie put(byte[] key, int value, boolean isRecursiveDelete) {
    ...
    private IndexTrie internalPut(byte[] key, int value, boolean isRecursiveDelete) {
        byte[] sharedPath = getSharedPath();
        byte[] commonPath = PackedTrieKeySlice.commonPath(key,sharedPath);
        if (PackedTrieKeySlice.length(commonPath) < PackedTrieKeySlice.length(sharedPath)) {
        ...
        byte pos =PackedTrieKeySlice.get(key,PackedTrieKeySlice.length(sharedPath));
        ...
        int keyLength =PackedTrieKeySlice.length(key);
        byte[] subKey = PackedTrieKeySlice.slice(key,
                PackedTrieKeySlice.length(sharedPath) + 1,
                 keyLength);
    ...
    private IndexTrie split(byte[] commonPath) {
        byte[] sharedPath = getSharedPath();
        int commonPathLength = PackedTrieKeySlice.length(commonPath);
        byte[] newChildSharedPath = PackedTrieKeySlice.slice(sharedPath,commonPathLength + 1, PackedTrieKeySlice.length(sharedPath));
        IndexTrie newChildTrie = newIndexTrie(newChildSharedPath, this.getValue(), this.getLeft(), this.getRight());

        // this bit will be implicit and not present in a shared path
        byte pos = PackedTrieKeySlice.get(sharedPath,commonPathLength);
    ...
    public byte[] getSharedPath() {
        return PackedTrieKeySlice.empty();
    }
    ...
    */
    ...
    private static class InOrderIterator implements Iterator<IterationElement> {
        ...
        public InOrderIterator(IndexTrie root) {
            ...
            byte[] traversedPath = root.getSharedPath();
        ...
        public IterationElement next() {
            ...
            if (rightNode != null) {
                byte[] rightNodeKey = PackedTrieKeySlice.rebuildSharedPath(visitingElement.getNodeKey(),(byte) 0x01, rightNode.getSharedPath());
         ...
         */
        private void pushLeftmostNode(byte[] nodeKey, IndexTrie node) {
            // find the leftmost node
            IndexTrie leftNode = node.retrieveNode((byte) 0x00);
            if (leftNode != null) {
                byte[] leftNodeKey = PackedTrieKeySlice.rebuildSharedPath(nodeKey,(byte) 0x00, leftNode.getSharedPath());
    ...
    private class PreOrderIterator implements Iterator<IterationElement> {
        ...
        public IterationElement next() {
            IterationElement visitingElement = visiting.pop();
            IndexTrie node = visitingElement.getNode();
            byte[] nodeKey = visitingElement.getNodeKey();
            // need to visit the left subtree first, then the right since a stack is a LIFO, push the right subtree first,
            // then the left
            IndexTrie rightNode = node.retrieveNode((byte) 0x01);
            if (rightNode != null) {
                byte[] rightNodeKey = PackedTrieKeySlice.rebuildSharedPath(nodeKey,(byte) 0x01, rightNode.getSharedPath());
    ...
    private class PostOrderIterator implements Iterator<IterationElement> {
         ...
         * @param node the root of the subtree for which we are trying to reach the leftmost node
         */
        private void pushLeftmostNodeRecord(byte[] nodeKey, IndexTrie node) {
            // find the leftmost node
            IndexTrie leftNode = node.retrieveNode((byte) 0x00);
            if (leftNode != null) {
                byte[] leftNodeKey = PackedTrieKeySlice.rebuildSharedPath(nodeKey,(byte) 0x00, leftNode.getSharedPath());
    ...
    public static class IterationElement {
        ...
        public IterationElement(final byte[] nodeKey, final IndexTrie node) {
        ...
        public final byte[] getNodeKey() {
            return nodeKey;
        }

        public String toString() {
            byte[] encodedFullKey = PackedTrieKeySlice.encode(nodeKey);
    ...
    private static class SharedPathSerializer {
        private final byte[] sharedPath;
        private final int lshared;

        private SharedPathSerializer(byte[] sharedPath) {
            this.sharedPath = sharedPath;
            this.lshared = PackedTrieKeySlice.length(this.sharedPath);
        }
    ...
    private List<IndexTrie> findNodes(byte[] key) {
        ...
        if (commonPathLength == PackedTrieKeySlice.length(key)) {
            List<IndexTrie> nodes = new ArrayList<>();
            nodes.add(this);
            return nodes;
        }

        IndexTrie node = this.retrieveNode(
                PackedTrieKeySlice.get(key,commonPathLength));

        if (node == null) {
            return null;
        }

        List<IndexTrie> subnodes = node.findNodes(
                PackedTrieKeySlice.slice(key,commonPathLength + 1,
                        PackedTrieKeySlice.length(key)));
